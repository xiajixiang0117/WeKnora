export default {
  ...
  datasource: {
    ...
    webCrawler: {
      ...
      allowedHosts: '允许的域名', allowedHostsPlaceholder: '留空则使用入口网址域名（多个用逗号分隔）', pathPrefixes: '路径前缀', pathPrefixesPlaceholder: '/projects/sdk/latest', excludePatterns: '排除规则', excludePlaceholder: '正则表达式，多个用逗号分隔', contentSelector: '正文 CSS 选择器', contentSelectorPlaceholder: '例如 #content 或 article[role="main"]', contentSelectorHint: '只导入与选择器匹配的 HTML 内容；留空时自动识别正文。', excludeSelectors: '排除 CSS 选择器', excludeSelectorsPlaceholder: '例如 .sidebar, .breadcrumbs', excludeSelectorsHint: '从选中的正文移除匹配节点，多个用逗号分隔。', maxPages: '单次最多页面', respectRobots: '遵守 robots.txt', batchHintTitle: '网站页面将按批次处理', batchHint: '保存后点击“检查更新”，系统先扫描并生成差异，审核通过后以每批 50 页的低优先级任务导入。', reviewTitle: '网站更新审核', checkUpdates: '检查更新', scanStarted: '更新检查已提交', scanFailed: '更新检查失败', loadFailed: '加载扫描结果失败', applyStarted: '批量应用任务已提交', applyFailed: '应用差异失败', retryFailed: '重试失败项', applySelected: '应用选中 ({count})', noChanges: '本次扫描没有发现差异', noScan: '还没有扫描记录', summary: '页面 {total} · 新增 {added} · 更新 {updated} · 缺失 {missing} · 失败 {failed}',
