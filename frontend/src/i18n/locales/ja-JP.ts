export default {
  ...
  sessionManagement: {
    documentCitationNote: "文書 URL への引用です。このチャンクは同じ出典ですが、引用された具体的なチャンクは特定できません。",
    viewChunk: "チャンク内容を展開",
    noChunks: "チャンクの記録なし",
    notSelected: "未選択",
    chunkUnavailable: "チャンク本文が保存されていないため、当時の内容を復元できません。",
    referenceContentNote: "回答に保存された引用から復元しました。結合された内容を含む場合があり、元の検索時点の本文ではありません。",
    viewAnswer: "回答を見る",
    finalAnswer: "最終回答",
    partialAnswer: "このリクエストは未完了です。生成済みの内容を表示します。",
    fallbackAnswer: "フォールバック回答です。",
    noAnswer: "生成された回答はありません。",
    answerUnavailable: "回答の記録や復元可能なメッセージがありません。",
    citedChunks: "回答で引用されたチャンク",
    citedChunksNote: "回答が明示的に引用したチャンクのみ表示します。モデルへの入力と引用は異なります。",
    noCitedChunks: "明示的な引用はありません。引用設定が無効か、回答が引用を含まない可能性があります。",
    contextUnavailable: "LLM に渡したチャンクは記録されていません。再ランク結果からは推定できません。",
    generationContext: "LLM に渡したチャンク",
    generationContextNote: "この段階でモデルに渡したチャンク（記録数 / 総数）",
  ...
  embedPublish: {
    ...
    originsHint: '埋め込みを許可するホストのオリジンを1行に1つ入力します（例: https://shop.example.com）。1件以上必須です。*.example.comのようなサブドメインのワイルドカードに対応しています',
