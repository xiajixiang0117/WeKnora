...
# Switch to the requested Debian mirror before every package operation. The
# configured sources use HTTP, so ca-certificates is not a prerequisite for
# selecting a mirror here.
RUN if [ -n "$APK_MIRROR_ARG" ]; then \
        sed -i "s@deb.debian.org@${APK_MIRROR_ARG}@g" /etc/apt/sources.list.d/debian.sources; \
    fi && \
    apt-get update && \
...
# Install other runtime packages using the selected mirror.
RUN apt-get update && \
