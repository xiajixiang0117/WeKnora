...
- PostgreSQL 的 `versioned/` 从 `000000_init` 到 `000094_mcp_metadata`；
- `sqlite/` 以 `000000_init` 作为压平后的全量初始化（JSONB→TEXT、SERIAL→AUTOINCREMENT 等方言差异已适配），其后按需追加增量版本（当前到 `000015_mcp_tool_enabled`），同样由 golang-migrate 顺序执行；
