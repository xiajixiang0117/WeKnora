...
func (t *SearchKnowledgeTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	recordAgentRetrievals(ctx, []string{query}, searchTargets, allResults)
...
func (t *SearchKnowledgeTool) rerankResults(
	...
	// Scoring mutates the embedded SearchResult; snapshot candidates first.
	candidates := unwrapSearchResults(results)
	...
	retrievaltrace.RecordRerank(ctx, "agent", query, t.rerankModel.GetModelID(), t.rerankModel.GetModelName(), threshold, candidates, scores, unwrapSearchResults(reranked), "completed", nil)
